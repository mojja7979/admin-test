...
import { EditOutlined, CloseSquareOutlined, CloseCircleOutlined, PlusSquareOutlined } from '@ant-design/icons';
import { isEmpty } from '../../lib/util';
import { useHistory } from "react-router-dom";
import { DragDropContext, Droppable, Draggable } from 'react-beautiful-dnd';


export const ChannelSnapshotDetailContainer = ({ match }: any) => {
  ...
  useEffect(() => {
    dispatch(asyncGetSnapshotDetail(id));
    ...
    if (isEmpty(title)) {
      ...
      asyncSaveSnapshotImage(formData).then((payload: any) => {
        ...
        asyncSaveSnapshotImages(formData).then((payload: any) => {
          ...
          for (var i in snapShotImages) {
            if (!snapShotImages[i].path) {
              paths.push({ imagePath: 'channel/snapshot' + snapShotImages[i].imagePath.split('channel/snapshot')[1], imgOrder: Number(i) })
            }
          }
          asyncEditSnapshot({
            data: {
  ...
  const getSnapshotList = () => {
    ...
    for (var i in snapshotList) {
      paths.push({
        imagePath: snapshotList[i].imagePath.split('cloudfront.net/')[1],
        imgOrder: i
      })
    }
              ...
              <td colSpan={3}>
                <div style={{ display: 'flex' }}>
                  <DragDropContext onDragEnd={onDragEnd}>
                    <Droppable droppableId="snapshots" direction="horizontal">
                      {(provided) => (
                        <div
                          className="snapshots"
